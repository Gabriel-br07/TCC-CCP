...
\section{Topologia Solução}
\label{sec:topologia-solucao}
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{latex-overleaf/04-figuras/topoliga-solucao.png}t
  \caption{Topologia Solução}
  \label{fig:topologia-solucao}
\end{figure}
\subsection{Atores e Casos de Uso Principais}

\textbf{Operador Judiciário} (Usuário Externo):

\textbf{Ação}: Inicia consultas via interface do chatbot ou API REST.
\textbf{Fluxo}: 
\item Envia perguntas ao \textbf{ChatbotInterface \& REST API} (Deployment + Ingress).
            \item Recebe respostas geradas pelo sistema, que combinam dados do \textbf{Vector DB} e processamento do \textbf{LLM externo} (OpenAI ou serviço privado).
        \end{itemize}
   
    \item \textbf{Administrador} (Integrado ao módulo \textbf{CI/CD \& MLOps}):
    \begin{itemize}
        \item \textbf{Ação}: Configura parâmetros da pipeline (ex.: frequência de coleta, modelos de embeddings).
        \item \textbf{Ferramentas}:
        \begin{itemize}
            \item \textbf{Tekton/Argo Workflows}: Orquestra pipelines de CI/CD.
            \item \textbf{GitOps (ArgoCD)}: Gerencia configurações e atualizações do cluster.
            \item \textbf{Container Registry}: Armazena imagens dos serviços.
        \end{itemize}
    \end{itemize}
    \item \textbf{Sistema} (Orquestrador Automático):
\textbf{Casos de Uso Principais (Include)}:
...
\textbf{DocumentCollector} → \textbf{Fila de Mensagens} → \textbf{Preprocessor} → \textbf{EmbeddingGenerator} → \textbf{Vector DB}.
        ...
        \item \textbf{Observação}: O fluxo é acionado periodicamente (CronJob) ou por eventos.
    \end{itemize}
\textbf{Serviços de Aplicação}:
    \begin{itemize}
        \item \textbf{ChatbotInterface \& REST API}: Expõe a funcionalidade para usuários.
        \item \textbf{RagService}: Coordena consultas semânticas e interação com o LLM.
    \end{itemize}
\textbf{Observabilidade}:
    \begin{itemize}
        \item \textbf{Prometheus/Grafana}: Métricas em tempo real.
        \item \textbf{Loki}: Armazenamento de logs.
        \item \textbf{Jaeger}: Rastreamento de transações.
    \end{itemize}
\textbf{CI/CD \& MLOps}:
Automatiza deploy, treinamento de modelos e atualizações via \textbf{Tekton/Argo Workflows} e \textbf{GitOps}.
