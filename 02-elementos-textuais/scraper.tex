...
\begin{itemize}
  ...
  \item seleção de modo de operação (por exemplo, RAG ou \textit{chat} puro) e de modelo; 
...
\begin{itemize}
  \item componente de página principal, responsável pela orquestração do fluxo de interação;
  \item componentes de apresentação (\textit{prompt}, painel de resultados, lista de fontes e elementos de utilidade);
  \item unidades de lógica reutilizáveis (\textit{composables} ou \textit{hooks}) que encapsulam chamadas a serviços, gerenciamento de estado e tratamento de erros;
  \item adaptadores para comunicação com a camada de serviços (API de backend) que fornecem respostas geradas, documentos recuperados e metadados.
\end{itemize}
