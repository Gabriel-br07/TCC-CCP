...
\subsection{Etapa 8 – Documentação e Preparação para Deployment}
Foram elaboradas documentações técnicas do código, manuais de instalação e configuração, e arquivos de conteinerização (Dockerfile) para facilitar a reprodutibilidade e o \textit{deployment} da solução. A documentação inclui descrição dos módulos, instruções de uso, exemplos de configuração e orientações para futuras extensões.
